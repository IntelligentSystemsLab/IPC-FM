...
def train_test(
    model_name, Meta_net, epoch, directory, directory_model, directory_other
):
    ...
    plt.savefig(directory + "/FedAvg_" + model_name + ".png")

    acc_l = pd.DataFrame(acc_l, columns=["FedAvg_" + model_name])
    acc_l.to_csv(directory + "/FedAvg_acc_" + model_name + ".csv")

    loss_t = pd.DataFrame(loss_t, columns=["FedAvg_" + model_name])
    loss_t.to_csv(directory + "/FedAvg_test_loss_" + model_name + ".csv")

    F1_score = pd.DataFrame(F1_score, columns=["FedAvg_" + model_name])
    F1_score.to_csv(directory_other + "/FedAvg_F1_score_" + model_name + ".csv")

    KAPPA = pd.DataFrame(KAPPA, columns=["FedAvg_" + model_name])
    KAPPA.to_csv(directory_other + "/FedAvg_KAPPA_" + model_name + ".csv")


def local_train(model_name, Meta_net, directory_model, directory_local):
    local_acc, local_LL, local_f1, local_kappa = Meta_net.local_train(
        directory_model, 5
    )

    local_acc = pd.DataFrame(local_acc, columns=["FedAvg_" + model_name])
    local_acc.to_csv(directory_local + "/FedAvg_local_acc_" + model_name + ".csv")

    local_LL = pd.DataFrame(local_LL, columns=["FedAvg_" + model_name])
    local_LL.to_csv(directory_local + "/FedAvg_local_LL_" + model_name + ".csv")

    local_f1 = pd.DataFrame(local_f1, columns=["FedAvg_" + model_name])
    local_f1.to_csv(directory_local + "/FedAvg_local_f1_" + model_name + ".csv")

    local_kappa = pd.DataFrame(local_kappa, columns=["FedAvg_" + model_name])
    local_kappa.to_csv(
        directory_local + "/FedAvg_local_kappa_" + model_name + ".csv"
    )
...
def main(model_name):
    ...
    print(model_num, noise_param)
    fixed_initial_net(model_num)
    print("Train model: FedAvg_" + str(model_name))

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    directory = str(model_num) + "/result/FedAvg_sgdmom" + model_name+ "_" + save_file
    directory_model = str(model_num) + "/result_model/FedAvg_sgdmom" + model_name+ "_" + save_file
    directory_local = str(model_num) + "/result_local_all/FedAvg_sgdmom" + model_name+ "_" + save_file
    directory_other = str(model_num) + "/result_other/FedAvg_sgdmom" + model_name+ "_" + save_file
    ...
    local_train(model_name, Meta_net,directory_model, directory_local)
