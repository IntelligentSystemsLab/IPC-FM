...
def model_summary(extra_betas_std, betas_extra, main_betas_std, betas_main,
                  X_vars_names=None, Q_vars_names=None):
    ...
    if not isinstance(extra_betas_std, float) and not isinstance(main_betas_std, float):
        ...
        stats_main = np.array(list(zip(X_vars_names, betas_main, main_betas_std, z_exog, p_exog)))
        stats_extra = np.array(list(zip(Q_vars_names, betas_extra, extra_betas_std, z_embs, p_embs)))
        stats_all = np.vstack([stats_main, stats_extra])
        df_stats = pd.DataFrame(index=[i[0] for i in stats_all],
                                data=np.array(
                                    [[np.float64(i[1]) for i in stats_all], [np.float64(i[2]) for i in stats_all],
                                     [np.float64(i[3]) for i in stats_all],
                                     [np.round(np.float64(i[4]), 4) for i in stats_all]]).T,
...
def get_betas_and_embeddings(trained_model, Q_df_train):
    ...
    DICT['index2alfa_from'], DICT['alfa2index_from'] = create_index(UNIQUE_CATS)
    betas_extra = trained_model.extra_model.conv1.weight.reshape(-1).cpu().detach().numpy()
    betas_main = trained_model.main_model.weight.reshape(-1).cpu().detach().numpy()
    embeddings = trained_model.extra_model.src_learning_emb.weight.cpu().detach().numpy()
