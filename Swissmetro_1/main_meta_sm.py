...
def train_test(model_name, Meta_net, inner_lr,outer_lr, directory, directory_model):
    epoch = args.epochs
    ...
    F1_score.to_csv(directory + '/FedMeta_F1_score' + str(inner_lr) + '_' + str(outer_lr) + '.csv')

    KAPPA = pd.DataFrame(KAPPA, columns=['FedMeta_' + model_name])
    KAPPA.to_csv(directory + '/FedMeta_KAPPA' + str(inner_lr) + '_' + str(outer_lr) + '.csv')
...
def main():
    ...
    # train_test(model_name, Meta_net, inner_lr, outer_lr, directory, directory_model)
