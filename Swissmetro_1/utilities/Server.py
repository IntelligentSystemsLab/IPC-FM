...
from utilities.network_logit import E_MNL, L_MNL, ASU_DNN, T_MNL, ET_MNL, MNL
...
class MetaServer(Server):
    def __init__(self, device, spt_prop, test_client_prop,
                 networkSize, hidden_layers,
                 beta_num, extra_feature, choices_num,
                 local_metatrain_epoch=1, local_metatest_epoch=2, batch_size=16, inner_lr=0.001, outer_lr=0.001,
                 mode="_fomaml", centralized=0, model_name='L_MNL', NUM_UNIQUE_CATS=81):
        ...
        dataset_main_all = np.array(dataset_main_all)
        dataset_extra_all = np.array(dataset_extra_all)
        self.saqure = saqure

        num_people_all = len(dataset_main_all)
        people_list = np.array(range(0, num_people_all, 1))
        train_id, test_id = train_test_split(people_list, test_size=test_client_prop, random_state=10)
        dataset_main_train = dataset_main_all[train_id, :, :, :]
        dataset_main_test = dataset_main_all[test_id, :, :, :]
        dataset_extra_train = dataset_extra_all[train_id, :, :]
        dataset_extra_test = dataset_extra_all[test_id, :, :]

        if centralized:
            model = deepcopy(self.net)
            data_main = dataset_main_train.reshape(len(train_id) * 9, self.beta_num + 1, self.choices_num)
            data_extra = dataset_extra_train.reshape(len(train_id) * 9, self.extra_feature)
            self.train_clients.append(MetaClient(
                device=self.device, id_c=-1, mode="_central", model=model,
                train_epoch=self.local_metatrain_epoch, test_epoch=self.local_metatest_epoch,
                batch_size=self.batch_size,
                inner_lr=inner_lr, outer_lr=outer_lr,data_main=data_main,
                data_extra=data_extra, model_name=model_name, spt_prop=spt_prop))
        else:
