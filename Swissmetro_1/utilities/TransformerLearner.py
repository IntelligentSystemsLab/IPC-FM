...
class Layer_M(nn.Module):
    ...
    def __init__(self, layer_n, n_layer1, dropout=0.1):
        ...
        self.Layer_M_model = nn.ModuleList([Different_layer(layer_n, layer_n, dropout_rate=dropout)
...
class ASU_T(nn.Module):

    def __init__(self, emb_vars_num, layer_nz, dropout=0.1):
        super(ASU_T, self).__init__()
        self.layer_z1 = Different_layer(emb_vars_num, layer_nz, dropout_rate=dropout)
        self.final_model = nn.Linear(layer_nz, 1)

    def forward(self, input_z):
        input_z = self.layer_z1(input_z)
        output = self.final_model(input_z)
        return output


class ASU(nn.Module):

    def __init__(self, cont_vars_num, layer_nx, layer_nz, layer_n2, n_layer1=0, n_layer2=0, dropout=0.1):
        ...
        if self.n_layer1 > 0:
            self.Layer_M1_x = Layer_M(
                layer_nx, self.n_layer1, dropout=dropout)

        self.Layer_M2_begin = Different_layer(layer_nx + layer_nz, layer_n2, dropout_rate=dropout)
        if self.n_layer2 > 0:
            self.Layer_M2 = Layer_M(layer_n2, self.n_layer2, dropout=dropout)

        self.final_model = nn.Linear(layer_n2, 1, bias=False)
