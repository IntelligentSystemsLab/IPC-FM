# -*- coding: UTF-8 -*-
"""
@Project :meta_lmnl @File :new_client.py
...
class MetaClient(Client):
    ...
    def local_fomaml_train(self, NN=False, ASU=False):
        self.init_model()
        self.init_net.train()
        for _ in range(self.train_epoch):
            for main_spt, extra_spt in zip(self.spt_main_loader_fo, self.spt_extra_loader_fo):
                ...
                loss_s.backward()
                nn.utils.clip_grad_norm_(self.init_net.parameters(), max_norm=50., norm_type=2)
                self.init_optim.step()
                with torch.no_grad():
                    self.regular_positive(self.init_net)
    ...
    def local_person(self):
        ...
        with torch.no_grad():
            for main_qry, extra_qry in zip(self.qry_main_loader_fo, self.qry_extra_loader_fo):
                ...
                output_c = torch.argmax(y_hat, dim=1)
                label_c = torch.argmax(qry_y, 1)
                query_loss.append(LL.item() * qry_x_main.size(0))
        return query_loss, output_c, label_c