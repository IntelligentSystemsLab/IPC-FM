...
parser.add_argument('--model', type=str, default="ET_MNL")
...
def train_test(model_name, Meta_net,inner_lr,directory,directory_model,directory_other):
    ...
    for i in range(epoch):
        ...
        F1_score.append(f1)
        KAPPA.append(kappa)
        print('Epoch: ' + str(i) + ' Loss:' + str(Loss) + ' Accuracy:' + str(accuracy) + ' Loss_test:' + str(LL_test))
    ...
    loss_C.to_csv(directory + '/central_train_loss_' + str(inner_lr) + model_name + '.csv')

    loss_t = pd.DataFrame(loss_t, columns=[model_name])
    loss_t.to_csv(directory + '/central_test_loss_' + str(inner_lr) + model_name + '.csv')

    F1_score = pd.DataFrame(F1_score, columns=[model_name])
    F1_score.to_csv(directory_other + '/central_F1_score_' + str(inner_lr) + model_name + '.csv')

    KAPPA = pd.DataFrame(KAPPA)
    KAPPA.to_csv(directory_other + '/central_KAPPA_' + str(inner_lr) + '.csv')
...
def local_train(model_name, Meta_net, inner_lr,directory_model, directory_local):
    ...
    local_acc.to_csv(directory_local + '/1_central_local_acc_' + model_name + str(inner_lr) + '.csv')

    local_LL = pd.DataFrame(local_LL, columns=['central_' + model_name])
    local_LL.to_csv(directory_local + '/1_central_local_LL_' + model_name + str(inner_lr) + '.csv')

    local_f1 = pd.DataFrame(local_f1, columns=['central_' + model_name])
    local_f1.to_csv(directory_local + '/1_central_local_f1_' + model_name + str(inner_lr) + '.csv')

    local_kappa = pd.DataFrame(local_kappa, columns=['central_' + model_name])
    local_kappa.to_csv(directory_local + '/1_central_local_kappa_' + model_name + str(inner_lr) + '.csv')
...
def main(model_name):
    ...
    directory = str(model_num) + '/result/sgdmom_' + model_name
    ...
    directory_other = str(model_num) + '/result_other/sgdmom_' + model_name
    ...
    Meta_net = MetaServer(device=device, centralized=1,
                          beta_num=beta_num, extra_feature=extra_feature,
                          ...
                          mode="_central", inner_lr=inner_lr,
                          spt_prop=conf['spt_prop'], test_client_prop=conf['test_client_prop'],
