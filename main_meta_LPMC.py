...
def train_test(model_name, Meta_net, inner_lr,outer_lr, directory, directory_model, directory_other):
    epoch = args.epochs
    ...
    plt.savefig(directory + '/FedMeta_' + model_name + '_' + str(inner_lr) + '_' + str(outer_lr) + '.png')

    acc_l = pd.DataFrame(acc_l, columns=['FedMeta_' + model_name])
    acc_l.to_csv(directory + '/FedMeta_acc' + str(inner_lr) + '_' + str(outer_lr) + '.csv')

    loss_t = pd.DataFrame(loss_t, columns=['FedMeta_' + model_name])
    loss_t.to_csv(directory + '/FedMeta_test_loss' + str(inner_lr) + '_' + str(outer_lr) + '.csv')

    F1_score = pd.DataFrame(F1_score, columns=['FedMeta_' + model_name])
    F1_score.to_csv(directory_other + '/FedMeta_F1_score' + str(inner_lr) + '_' + str(outer_lr) + '.csv')

    KAPPA = pd.DataFrame(KAPPA, columns=['FedMeta_' + model_name])
    KAPPA.to_csv(directory_other + '/FedMeta_KAPPA' + str(inner_lr) + '_' + str(outer_lr) + '.csv')


def local_train(model_name, Meta_net, inner_lr,outer_lr, directory_model, directory_local):
    ...
    local_acc.to_csv(directory_local + '/1_FedMeta_local_acc_' + str(inner_lr) + '_' + str(outer_lr) + '.csv')

    local_LL = pd.DataFrame(local_LL, columns=['FedMeta_' + model_name])
    local_LL.to_csv(directory_local + '/1_FedMeta_local_LL_' + str(inner_lr) + '_' + str(outer_lr) + '.csv')

    local_f1 = pd.DataFrame(local_f1, columns=['FedMeta_' + model_name])
    local_f1.to_csv(directory_local + '/1_FedMeta_local_f1_' + str(inner_lr) + '_' + str(outer_lr) + '.csv')

    local_kappa = pd.DataFrame(local_kappa, columns=['FedMeta_' + model_name])
    local_kappa.to_csv(directory_local + '/1_FedMeta_local_kappa_' + str(inner_lr) + '_' + str(outer_lr) + '.csv')
...
def main():
    ...
    directory_local = str(model_num) + '/result_local_all/FedMeta_sgd_sgdmom' + model_name  # sgd  sgdmom   adam
    ...
    Meta_net = MetaServer(device=device, beta_num=beta_num, extra_feature=extra_feature,
                          NUM_UNIQUE_CATS=NUM_UNIQUE_CATS, choices_num=choices_num,
                          networkSize=conf['networkSize'], hidden_layers=conf['hidden_layers'],
                          batch_size=conf['batchSize'], mode="_fomaml",
                          spt_prop=conf['spt_prop'], model_name=model_name, inner_lr=inner_lr, outer_lr=outer_lr,
                          test_client_prop=conf['test_client_prop'])  # sgd:0.1 adam:0.1
    # train_test(model_name, Meta_net, inner_lr, outer_lr, directory, directory_model, directory_other)
    local_train(model_name, Meta_net, inner_lr, outer_lr, directory_model, directory_local)
